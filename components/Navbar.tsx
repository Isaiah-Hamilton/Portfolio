...
const Navbar = () => {
  ...
  return (
    <nav
      className="w-full fixed top-0 p-4 sm:px-6 md:px-10 lg:px-20 z-30"
      style={{
        backdropFilter: 'blur(24px)',
        backgroundColor: '#e9e8e469',
        opacity: 1,
      }}
    >
      ...
      >
        {NavLinks.map((item: NavLinkTypes, i: number) => (
          <motion.div
            ...
            transition={{
              delay: isOpen ? i * 0.1 : (NavLinks.length - i) * 0.1,
            }}
          >
            <Button onClick={() => setIsOpen(false)} noStyle link={item.url}>
              <div className="p-3 rounded-xl hover:bg-[#e9e8e478] transition">{item.text}</div>
            </Button>
          </motion.div>
        ))}
      </motion.div>
