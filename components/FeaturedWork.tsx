  ...
  <Section id="work">
    ...
    <div className="space-y-8 md:space-y-10">
      {Projects.map((item: ProjectTypes, i: number) => (
        <div
          key={item.title}
          className="w-full h-[575px] rounded-3xl flex flex-col-reverse lg:flex-row lg:items-center lg:sticky"
          style={{
            top: `${100 + i * 15}px`,
        ...
        >
          <div className="lg:w-1/2 p-6 md:p-8">
            ...
            <p className="sm:w-4/5 md:w-[95%] lg:w-full mt-4 mb-10 font-light text-xs md:text-base">
              {item.description}
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {item.tags.map((tag: string) => (
                <div
                  key={tag}
                  className={`inline-block border-[1px] ${item.textColor ? 'border-[#303030] text-[#ffffffb2]' : 'border-[#8a8a8a99] text-[#222]'} text-center text-xs font-light rounded-full px-3 py-1`}
                >
                  {tag}
                </div>
              ))}
