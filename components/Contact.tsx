      ...
      <Section id='contact' className='relative z-20 !pb-0'>
        ...
        <div className='md:w-4/5 xl:w-3/4 2xl:w-1/2 mx-auto space-y-4'>
          <div className='grid grid-rows-3 md:grid-rows-none md:grid-cols-3 gap-4'>
            ...
            <Button className='col-span-1' link="https://twitter.com/isaiah7hamilton" target='_blank'>
              <div className='flex items-center w-fit mx-auto'>
                <span>DM me</span>
                <IconTwitter />
              </div>
          ...
          <div>
            <div className='grid grid-rows-2 md:grid-rows-none md:grid-cols-2 gap-4'>
              <Input className='col-span-1' type="text" placeholder='Name' required value={name} onChange={(event) => setName(event.target.value)} />
              <Input className='col-span-1' type="email" placeholder='Email' required value={email} onChange={(event) => setEmail(event.target.value)} />
            </div>
            <Textarea placeholder='Message' required value={message} onChange={(event) => setMessage(event.target.value)} />
            <Button onClick={() => SendEmail(name, email, message)} disabled={!name || !email || !message || loding ? true : false} className='w-full mt-4 py-4'>{buttonText}</Button>
          </div>
        </div>
      </Section>
    </div>
